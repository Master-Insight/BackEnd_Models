...
// sendResponse genera y envía una respuesta genérica
func sendResponse(c echo.Context, code int, data interface{}, message string) error {
	if message == "" {
...
func Unprocessable(c echo.Context, message string, data interface{}) error {
	return sendResponse(c, http.StatusUnprocessableEntity, data, message)
}
...
func ServerError(c echo.Context, message string, data interface{}) error {
	return sendResponse(c, http.StatusInternalServerError, data, message)
}
